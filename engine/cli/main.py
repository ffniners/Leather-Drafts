...
def construct(inp: Path = typer.Option(..., exists=True, dir_okay=False, help="Base JSON"),
    ...
    """Apply construction transforms (seam allowance, hems, etc.). Adds `sa_paths` per piece."""
    import yaml
    from ..transforms.seam_allowance import offset_piece
    out.parent.mkdir(parents=True, exist_ok=True)
    data = json.loads(Path(inp).read_text())
    opts = yaml.safe_load(Path(options).read_text())
